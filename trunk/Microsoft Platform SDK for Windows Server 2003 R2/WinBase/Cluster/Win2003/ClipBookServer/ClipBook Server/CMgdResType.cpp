    ...
    )
{
    ...
    hr = HrSendStatusReport(
        ...
        , TASKID_Minor_ClipBookServer_ResType_CommitChanges
        , 0
        , 6
        , 0
        , hr
        , IDS_CLUSCFG_INFO_CONFIGURING_RESTYPE
        , 0
        , GetDisplayName()
        );
    if ( FAILED( hr ) )
    ...
    if ( ( ecm == cmCREATE_CLUSTER ) || ( ecm == cmADD_NODE_TO_CLUSTER ) )
    {
        ...
        hr = HrSendStatusReport(
            ...
            , 4
            , hr
            , IDS_CLUSCFG_INFO_CONFIGURED_RESTYPE
            , 0
            , GetResTypeName()
            , GetDisplayName()
            , GetDllName()
...
Cleanup:
    ...
    if ( FAILED( hr ) )
    {
        HrSendStatusReport(
            ...
            , 6
            , hr
            , IDS_CLUSCFG_ERROR_CONFIGURING_RESTYPE_FAILED
            , 0
            , GetDisplayName()
            );
    } // if: FAILED
    ...
    )
{
    ...
    {
        *pguidGUIDOut = RESTYPE_ClipBookServer;
    ...
    )
{
    ...
    const GUID *                    guidAdminEx = &CLSID_CoClipBookServerEx;
    ...
    hr = pcrCatReg->UnRegisterClassImplCategories( CLSID_ClipBookServer, RTL_NUMBER_OF( rgCatId ), rgCatId );
    if ( FAILED( hr ) )
    {
        goto Cleanup;
