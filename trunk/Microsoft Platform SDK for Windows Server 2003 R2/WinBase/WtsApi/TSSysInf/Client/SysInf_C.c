...
void DisplayError(LPTSTR pszAPI)
{
    ...
    //
    i = wsprintf(buffer,      TEXT("SYSINF_C ERROR: API        = %s.\n"), pszAPI);
    i += wsprintf(buffer + i, TEXT("                error code = %u.\n"), lastErr);
    wsprintf(buffer + i,      TEXT("                message    = %s.\n"), (LPTSTR)lpvMessageBuffer);
...
void GetVersionInfo(void)
{
    ...
    // get the client name
    //
    strcpy(tsvi.szClientName, gszClientName);
    ...
    lstrcpy(tsvi.szComputerName, szComputerName);
    ...
    lstrcpy(tsvi.szUserName, szUserName);
...
VOID VCAPITYPE VirtualChannelInitEventProc(LPVOID pInitHandle, UINT event, LPVOID pData, UINT dataLength)
{
    ...
    switch(event)
    {
        ...
        case CHANNEL_EVENT_CONNECTED:
        {
            ...
            strcpy(gszClientName, (LPSTR)pData);
