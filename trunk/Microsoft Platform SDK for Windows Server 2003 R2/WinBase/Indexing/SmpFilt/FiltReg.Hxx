    ...
    do
    {
        ...
        if ( 0 != awszKeyValues[i] )
            dwError = RegSetValueExW( hKey,                     // Key
                                      ...
                                      (DWORD) ( (1 + wcslen(awszKeyValues[i]) ) * sizeof(WCHAR) ) );
...
inline long DestroyKeyValues( WCHAR const * const * awszKeyValues, int cKeyValues )
{
    ...
    unsigned cc = (unsigned) wcslen( wcTemp );
    ...
    for ( i -= 2; i >= 0; i -= 2 )
    {
        ...
        cc -= (unsigned) wcslen( awszKeyValues[i] );
        cc--;
                                  ...
                                  BOOL fBlastExistingPersistentHandler )
{
    ...
    do
    {
        ...
        if ( 0 == Class.pwszExt )
        {
            ...
            if ( wcslen( Class.pwszShortName ) >= MAX_PATH )
        ...
        {
            if ( wcslen( Class.pwszExt ) >= MAX_PATH )
                return ERROR_INSUFFICIENT_BUFFER;
            ...
            if ( fSetValue && !fAppendOnly )
            {
                dwError = RegSetValueExW( hKey,                        // Key
                                          ...
                                          (DWORD) ( (1 + wcslen(Class.pwszShortName) ) * sizeof(WCHAR) ) );
            ...
            if ( fBlastExistingPersistentHandler ||
                ...
                dwError = RegSetValueExW( hKey2,                 // Key
                                          ...
                                          (DWORD) ( (1 + wcslen(pwszPHandler) ) * sizeof(WCHAR) ) );
                            ...
                            BOOL fAppendOnly )
{
    ...
    {
        if ( wcslen( pwszShortName ) >= MAX_PATH )
            return ERROR_INSUFFICIENT_BUFFER;
    ...
    do
    {
        ...
        if ( REG_CREATED_NEW_KEY == dwDisposition && 0 == pwszShortName )
        {
            dwError = RegSetValueExW( hKey,                          // Key
                                      ...
                                      (DWORD) ( (1 + wcslen(Class.pwszDescription) ) * sizeof(WCHAR) ) );
        ...
        if ( ERROR_SUCCESS != dwError )
        {
            if ( ERROR_FILE_NOT_FOUND == dwError )
            {
                ...
                dwError = RegSetValueExW( hKey,                          // Key
                                          ...
                                          (DWORD) ( (1 + wcslen(Class.pwszClassId) ) * sizeof(WCHAR) ) );
    ...
    do
    {
        ...
        if ( REG_CREATED_NEW_KEY == dwDisposition && 0 == pwszClassId && fAppendDesc )
        {
            dwError = RegSetValueExW( hKey,                                 // Key
                                      ...
                                      (DWORD) ( (1 + wcslen(Class.pwszClassIdDescription) ) * sizeof(WCHAR) ) );
...
inline long RemoveAClassName( WCHAR * pwszClass )
{
    ...
    long dwIgnore = ERROR_SUCCESS;  // For non-fatal errors
