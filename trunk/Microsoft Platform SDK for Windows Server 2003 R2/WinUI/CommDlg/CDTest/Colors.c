...
UINT_PTR CALLBACK ColorsHookProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) ;
...
/************************************************************************
  ...
  Function: ColorsProc(HWND, UINT, WPARAM, LPARAM)
...
INT_PTR APIENTRY ColorsProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  BOOL bRet = FALSE ;
...
void InitColorStruct(HWND hwnd, LPCHOOSECOLOR pcc)
{
  ...
  lstrcpyn(szTemplateNameC, TEXT("clrtemp"), ARRAYSIZE(szTemplateNameC)) ;         //name of custom template
...
void FillColorDlg(HWND hwnd, LPCHOOSECOLOR pcc)
{
  ...
  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pcc->hwndOwner) ;
  SetDlgItemText(hwnd, ID_HWNDOWNERC, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pcc->hInstance) ;
  ...
  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pcc->lpfnHook) ;
...
void GetColorDlg(HWND hwnd, LPCHOOSECOLOR pcc)
{
  ...
  pcc->lStructSize = (DWORD) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;
  ...
  pcc->rgbResult = (COLORREF) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;
  ...
  for (i=0; i<16; i++)
  {
    if (SendDlgItemMessage(hwnd, ID_CUSTCOLORSC, CB_GETLBTEXT,
      ...
      dwCustColors[i] = (DWORD) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;
  ...
  pcc->Flags = (DWORD) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;
...
/************************************************************************
  ...
  Function: ColorsHookProc(HWND, UINT, WPARAM, LPARAM) ;
