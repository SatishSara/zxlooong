...
UINT_PTR CALLBACK FindReplaceHookProc(HWND, UINT, WPARAM, LPARAM) ;
...
/************************************************************************
  ...
  Function: FindProc(HWND, UINT, WPARAM, LPARAM)
...
INT_PTR APIENTRY FindProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  switch (msg)
  {
    case WM_INITDIALOG:
...
void InitFindStruct(HWND hwnd, LPFINDREPLACE pfr)
{
  ...
  lstrcpyn(szFindWhat, TEXT("Word to find"), ARRAYSIZE(szFindWhat)) ;
  pfr->lpstrFindWhat = szFindWhat ;
  pfr->wFindWhatLen = 100 ;

  lstrcpyn(szReplaceWith, TEXT("Replace with word"), ARRAYSIZE(szReplaceWith)) ;
  ...
  lstrcpyn(szTemplate, TEXT("fttemp1"), ARRAYSIZE(szTemplate)) ;
...
void FillFindDlg(HWND hwnd, LPFINDREPLACE pfr)
{
  ...
  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pfr->hwndOwner) ;
  SetDlgItemText(hwnd, ID_HWNDOWNERFT, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pfr->hInstance) ;
  ...
  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pfr->lpfnHook) ;
...
void GetFindDlg(HWND hwnd, LPFINDREPLACE pfr)
{
  ...
  pfr->lStructSize = (DWORD) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;
  ...
  pfr->Flags = (DWORD) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;
...
/************************************************************************
  ...
  Function: FindReplaceHookProc(HWND, UINT, WPARAM, LPARAM)
...
UINT_PTR CALLBACK FindReplaceHookProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  LPFINDREPLACE pFr ;
  TCHAR szMsg[75] ;
  TCHAR szTmp[20] ;
  ...
  switch(msg)
  {
    case WM_INITDIALOG:
      ...
      if (GetDlgItem(hwnd, ID_DEFSTRINGS))
      {
        for (i=0; i<5; i++)
        {
          lstrcpyn(szMsg, szDefString, ARRAYSIZE(szMsg)) ;
          wsprintf(szTmp, TEXT("Number %d"), i+1) ;
          StrCatBuff(szMsg, szTmp, ARRAYSIZE(szTmp)) ;
          SendDlgItemMessage(hwnd, ID_DEFSTRINGS, LB_ADDSTRING, (WPARAM) 0,
                             (LONG_PTR) (LPTSTR) szMsg) ;
        }
      }
