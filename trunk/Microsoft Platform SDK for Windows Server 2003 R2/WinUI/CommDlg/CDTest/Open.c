...
UINT_PTR CALLBACK OpenSaveHookProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) ;
...
HRSRC  hRes ;           //handles to the resource and dialog for
...
/************************************************************************
  ...
  Function: OpenFunc(HWND, UINT, WPARAM, LPARAM)
...
INT_PTR APIENTRY OpenFunc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  switch (msg)
  {
...
void InitOpenStruct(HWND hwnd, LPOPENFILENAME po)
{
   ...
   if (bDoOpenDlg)
     lstrcpyn(szDlgTitle, TEXT("Open Dialog Title"), ARRAYSIZE(szDlgTitle)) ;
   else
     lstrcpyn(szDlgTitle, TEXT("Save Dialog Title"), ARRAYSIZE(szDlgTitle)) ;

   lstrcpyn(szDefExt, TEXT("rat"), ARRAYSIZE(szDefExt)) ;
   lstrcpyn(szInitialDir, TEXT("c:\\"), ARRAYSIZE(szInitialDir)) ;
   lstrcpyn(szTempName, TEXT("opentemp1"), ARRAYSIZE(szTempName)) ;

   lstrcpyn(&szFilterInits[0][0], TEXT("All Files (*.*)"), 30) ;
   lstrcpyn(&szFilterInits[1][0], TEXT("*.*"), 30) ;
   lstrcpyn(&szFilterInits[2][0], TEXT("Fat Files (*.fat)"), 30) ;
   lstrcpyn(&szFilterInits[3][0], TEXT("*.fat"), 30) ;
   szFilterInits[4][0] = (TCHAR) 0 ;

   lstrcpyn(&szCustFiltInits[0][0], TEXT("Last Filter Used"), 30) ;
   lstrcpyn(&szCustFiltInits[1][0], TEXT("*.lst"), 30) ;
...
void FillOpenDlg(HWND hwnd, LPOPENFILENAME po)
{
   ...
   wsprintf(szTemp, szPointerFilter, (LONG_PTR) po->hwndOwner) ;
   SetDlgItemText(hwnd, ID_HWNDOWNERO, szTemp) ;

   wsprintf(szTemp, szPointerFilter, (LONG_PTR) po->hInstance) ;
...
void GetOpenDlg(HWND hwnd, LPOPENFILENAME po)
{
   ...
   po->lStructSize = (DWORD) MyAtol(szNum, uMode==IDM_HEXMODE, &b) ;
   ...
   po->Flags = (DWORD) MyAtol(szNum, uMode==IDM_HEXMODE, &b) ;
...
void InitFilterString(void)
{
  ...
  while (szFilterInits[i][0] != (TCHAR) 0)
  {
    lstrcpyn(lpStr, &szFilterInits[i][0], MAXBUF) ;
...
void InitCustFilterString(void)
{
  ...
  {
    lstrcpyn(lpStr, &szCustFiltInits[i][0], MAXBUF) ;
...
void InterpretCustomFilterString(void)
{
  ...
  lstrcpyn(&szCustFiltInits[0][0], ofn.lpstrCustomFilter, 30) ;
  lstrcpyn(&szCustFiltInits[1][0], pNext, 30) ;
...
/************************************************************************
  ...
  Function: OpenSaveHookProc(HWND, UINT, WPARAM, LPARAM) ;
