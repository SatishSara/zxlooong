...
INT_PTR APIENTRY LogFontProc(HWND, UINT, WPARAM, LPARAM) ;
UINT_PTR CALLBACK FontHookProc(HWND, UINT, WPARAM, LPARAM) ;
int  CALLBACK EnumFontsProc(LPLOGFONT, LPTEXTMETRIC, DWORD, LPARAM) ;
...
/************************************************************************
  ...
  Function: FontProc(HWND, UINT, WPARAM, LPARAM)
...
INT_PTR APIENTRY FontProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  BOOL b ;
  TCHAR szNum[30] ;
...
void InitFontStruct(HWND hwnd, LPCHOOSEFONT pcf)
{
  ...
  lstrcpyn(szFontTemplate, TEXT("fonttemp"), ARRAYSIZE(szFontTemplate)) ;
  pcf->lpTemplateName = szFontTemplate ;

  pcf->hInstance = (HANDLE) hInst ;

  lstrcpyn(szFontStyle, TEXT("Bold"), ARRAYSIZE(szFontStyle)) ;
...
void FillFontDlg(HWND hwnd, LPCHOOSEFONT pcf)
{
  ...
  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pcf->hwndOwner) ;
  SetDlgItemText(hwnd, ID_HWNDOWNERF, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pcf->hDC) ;
  SetDlgItemText(hwnd, ID_HDCF, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pcf->hDC) ;
  SetDlgItemText(hwnd, ID_HDCF, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pcf->lpLogFont) ;
  SetDlgItemText(hwnd, ID_LOGFONTF, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pcf->iPointSize) ;
  ...
  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pcf->lpfnHook) ;
  SetDlgItemText(hwnd, ID_HOOKF, szTemp) ;

  SetDlgItemText(hwnd, ID_TEMPLATEF, pcf->lpTemplateName) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) pcf->hInstance) ;
...
#define WSIZEFO 30
  ...
  pcf->lStructSize = (DWORD) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;
  ...
  pcf->Flags = (DWORD) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;

  GetDlgItemText(hwnd, ID_RGBCOLORSF, szNum, WSIZEFO) ;
  pcf->rgbColors = (COLORREF) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;
  ...
  pcf->nFontType = (WORD) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;
...
/************************************************************************
  ...
  Function: LogFontProc(HWND, int, WPARAM, LPARAM)
...
INT_PTR APIENTRY LogFontProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  switch (msg)
  {
    case WM_INITDIALOG:
...
void InitLogFontStruct(HWND hwnd, LPLOGFONT plf)
{
  ...
  lstrcpyn(plf->lfFaceName, szFaceName, ARRAYSIZE(plf->lfFaceName)) ;
}
...
void GetLogFontDlg(HWND hwnd, LPLOGFONT plf)
{
  ...
  plf->lfHeight = (LONG) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;

  GetDlgItemText(hwnd, ID_LFWIDTH, szNum, LFSIZE) ;
  plf->lfWidth = (LONG) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;

  GetDlgItemText(hwnd, ID_LFESCAPEMENT, szNum, LFSIZE) ;
  plf->lfEscapement = (LONG) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;

  GetDlgItemText(hwnd, ID_LFORIENTATION, szNum, LFSIZE) ;
  plf->lfOrientation = (LONG) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;

  GetDlgItemText(hwnd, ID_LFWEIGHT, szNum, LFSIZE) ;
  plf->lfWeight = (LONG) MyAtol(szNum, uMode == IDM_HEXMODE, &b) ;
...
/************************************************************************
  ...
  Function: FontHookProc(HWND, UINT, WPARAM, LPARAM)
                           ...
                           DWORD dwStyle, LPARAM lParam)
{
  ...
  if (*(lplf->lfFaceName))
    SendDlgItemMessage((HWND) lParam, ID_FONTLIST, LB_ADDSTRING,
                     (WPARAM) 0, (LPARAM) (LPTSTR) lplf->lfFaceName) ;
...
void FillFontsBox(HWND hwnd, DWORD dwFlags)
{
  ...
  if (dwFlags & CF_SCREENFONTS)
  {
    ...
    EnumFontFamilies(hdc, (LPTSTR) NULL, (FONTENUMPROC) EnumFontsProc, (LONG_PTR) hwnd) ;
    ReleaseDC(hwnd, hdc) ;
  }
