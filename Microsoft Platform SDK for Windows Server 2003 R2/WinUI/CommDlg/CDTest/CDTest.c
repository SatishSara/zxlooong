...
/************************************************************************
  ...
  Function: HandleTheCommand(HWND, WPARAM, LPARAM)
...
void HandleTheCommand(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (LOWORD(wParam))
  {
    ...
    case IDM_DECIMALMODE:
      ...
      CheckMenuItem(GetMenu(hWnd), (UINT)uMode, MF_UNCHECKED) ;
      uMode = wParam ;
      CheckMenuItem(GetMenu(hWnd), (UINT)uMode, MF_CHECKED) ;
      ...
      {
        lstrcpyn(szShortFilter, TEXT("%x"), ARRAYSIZE(szShortFilter)) ;
        lstrcpyn(szLongFilter, TEXT("%lx"), ARRAYSIZE(szLongFilter)) ;
        lstrcpyn(szPointerFilter, TEXT("%p"), ARRAYSIZE(szPointerFilter)) ;
      }
      if (uMode == IDM_DECIMALMODE)
      {
        lstrcpyn(szShortFilter, TEXT("%d"), ARRAYSIZE(szShortFilter)) ;
        lstrcpyn(szLongFilter, TEXT("%ld"), ARRAYSIZE(szLongFilter)) ;
        lstrcpyn(szPointerFilter, TEXT("%Id"), ARRAYSIZE(szPointerFilter)) ;
      }
...
LONG_PTR MyAtol(LPTSTR szString, BOOL bHex, LPBOOL bSuccess)
{
  LPTSTR p ;
  LONG l ;
  LONG lMultiplier ;
...
INT_PTR APIENTRY AboutProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
