...
extern UINT_PTR uMode ;
extern LONG_PTR MyAtol(LPTSTR, BOOL, LPBOOL) ;
UINT_PTR CALLBACK PrintHookProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) ;
...
INT_PTR APIENTRY PrintProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  switch (msg)
  {
    case WM_INITDIALOG:
...
/************************************************************************
  ...
  Function: DevnamesProc(HWND, UINT, WPARAM, LPARAM)
...
INT_PTR APIENTRY DevnamesProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  switch (msg)
  {
    case WM_INITDIALOG:
...
/************************************************************************
  ...
  Function: DevmodeProc(HWND, UINT, WPARAM, LONG)
...
INT_PTR APIENTRY DevmodeProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  switch (msg)
  {
    case WM_INITDIALOG:
...
void InitPrintStruct(HWND hwnd, LPPRINTDLG ppd)
{
  ...
  lstrcpyn(szPrintTempName, TEXT("prtemp1"), ARRAYSIZE(szPrintTempName)) ;
  ppd->lpPrintTemplateName = szPrintTempName ;

  lstrcpyn(szSetupTempName, TEXT("prtemp2"), ARRAYSIZE(szSetupTempName)) ;
...
void FillPrintDlg(HWND hwnd, LPPRINTDLG ppd)
{
  ...
  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) ppd->hwndOwner) ;
  SetDlgItemText(hwnd, ID_HWNDOWNERP, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) ppd->hDevMode) ;
  SetDlgItemText(hwnd, ID_HDEVMODEP, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) ppd->hDevNames) ;
  SetDlgItemText(hwnd, ID_HDEVNAMESP, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) ppd->hDC) ;
  SetDlgItemText(hwnd, ID_HDCP, szTemp) ;

  wsprintf(szTemp, szLongFilter, ppd->Flags) ;
  SetDlgItemText(hwnd, ID_FLAGSP, szTemp) ;

  wsprintf(szTemp, szShortFilter, ppd->nFromPage) ;
  SetDlgItemText(hwnd, ID_FROMPAGEP, szTemp) ;

  wsprintf(szTemp, szShortFilter, ppd->nToPage) ;
  SetDlgItemText(hwnd, ID_TOPAGEP, szTemp) ;

  wsprintf(szTemp, szShortFilter, ppd->nMinPage) ;
  SetDlgItemText(hwnd, ID_MINPAGEP, szTemp) ;

  wsprintf(szTemp, szShortFilter, ppd->nMaxPage) ;
  SetDlgItemText(hwnd, ID_MAXPAGEP, szTemp) ;

  wsprintf(szTemp, szShortFilter, ppd->nCopies) ;
  SetDlgItemText(hwnd, ID_COPIESP, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) ppd->hInstance) ;
  SetDlgItemText(hwnd, ID_HINSTANCEP, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) ppd->lCustData) ;
  SetDlgItemText(hwnd, ID_CUSTDATAP, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) ppd->lpfnPrintHook) ;
  SetDlgItemText(hwnd, ID_PRINTHOOKP, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) ppd->lpfnSetupHook) ;
  ...
  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) ppd->hPrintTemplate) ;
  SetDlgItemText(hwnd, ID_HPRINTTEMPP, szTemp) ;

  wsprintf(szTemp, szPointerFilter, (DWORD_PTR) ppd->hSetupTemplate) ;
...
void GetPrintDlg(HWND hwnd, LPPRINTDLG ppd)
{
  ...
  ppd->lStructSize = (DWORD) MyAtol(szNum, uMode==IDM_HEXMODE, &b) ;
  ...
  ppd->Flags = (DWORD) MyAtol(szNum, uMode==IDM_HEXMODE, &b) ;
...
void GetDevnamesDlg(HWND hwnd, LPPRINTDLG ppd)
{
  ...
  lstrcpyn((LPTSTR) pn + pn->wDriverOffset, (LPTSTR) sz1, (dwSize / sizeof(TCHAR)) - pn->wDriverOffset) ;



  /* skip over string 1 and add string 1 with a zero between them */

  pn->wDeviceOffset = pn->wDriverOffset + (WORD)n1 + 1 ;
  lstrcpyn((LPTSTR) pn + pn->wDeviceOffset, (LPTSTR) sz2, (dwSize / sizeof(TCHAR)) - pn->wDeviceOffset) ;



  /* skip over string 2 and put string 3 there with a zero between them */

  pn->wOutputOffset = pn->wDeviceOffset + (WORD)n2 + 1 ;
  lstrcpyn((LPTSTR) pn + pn->wOutputOffset, (LPTSTR) sz3, (dwSize / sizeof(TCHAR)) - pn->wOutputOffset) ;
...
void GetDevmodeDlg(HWND hwnd, LPPRINTDLG ppd)
{
  ...
  GetDlgItemText(hwnd, ID_DMDEVNAME, (LPTSTR)p->dmDeviceName, 32) ;
  ...
  p->dmFields = (DWORD) MyAtol(szNum, uMode==IDM_HEXMODE, &b) ;
  ...
  GetDlgItemText(hwnd, ID_DMFORMNAME, (LPTSTR)p->dmFormName, 32) ;
  ...
  p->dmPelsWidth = (DWORD) MyAtol(szNum, uMode==IDM_HEXMODE, &b) ;

  GetDlgItemText(hwnd, ID_DMPELSHEIGHT, szNum, WMAXDEV) ;
  p->dmPelsHeight = (DWORD) MyAtol(szNum, uMode==IDM_HEXMODE, &b) ;

  GetDlgItemText(hwnd, ID_DMDISPLAYFREQ, szNum, WMAXDEV) ;
  p->dmDisplayFrequency = (DWORD) MyAtol(szNum, uMode==IDM_HEXMODE, &b) ;
...
/************************************************************************
  ...
  Function: PrintHookProc(HWND, UINT, WPARAM, LPARAM)
...
UINT_PTR CALLBACK PrintHookProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  return 0 ;   //send msg to the common dialog code
}
