...
HANDLE hInst;
TCHAR str[255] = {0};                       /* general-purpose string buffer */
    ...
    switch (message)
    {
        ...
        case WM_CHAR:
            if (wParam == '\r')
            {
                ...
                _tcsncpy (str, TEXT("Calculating prime numbers..."), sizeof(str)/sizeof(str)-1);
                InvalidateRect (hWnd, NULL, TRUE);
                UpdateWindow (hWnd);
                _sntprintf (str, sizeof(str)/sizeof(str)-1, TEXT("Calculated %d primes.       "), sieve());
                InvalidateRect (hWnd, NULL, TRUE);
                UpdateWindow (hWnd);
        ...
        case WM_LBUTTONDOWN:
            bTrack = TRUE;
            _tcsncpy (str,TEXT(""),sizeof(str)/sizeof(str)-1 );
            PrevX = LOWORD(lParam);
            PrevY = HIWORD(lParam);
            if (!(wParam & MK_SHIFT))
