...
#define SMX_OFFSETOF(x)	        ((UINT_PTR)(&((PSAMPLEDROPEXT)0)->x))
...
//
LPCSTR _Offset2Ptr(LPSTR pszBase, UINT_PTR offset, UINT * pcb)
{
    LPSTR pszRet;
    if (offset==0) {
	pszRet = NULL;
...
{
    FORMATETC fmte = {
        	(CLIPFORMAT)(g_cfNetResource ? g_cfNetResource
...
STDMETHODIMP_(ULONG) DropExt_CTM_AddRef(LPCONTEXTMENU pctm)
{
    PSAMPLEDROPEXT this = PCTM2PSMX(pctm);
    return ++this->_cRef;
}
...
STDMETHODIMP_(ULONG) DropExt_SXI_AddRef(LPSHELLEXTINIT psxi)
{
    PSAMPLEDROPEXT this = PSXI2PSMX(psxi);
    return ++this->_cRef;
}
...
STDMETHODIMP_(ULONG) DropExt_CTM_Release(LPCONTEXTMENU pctm)
{
    PSAMPLEDROPEXT this = PCTM2PSMX(pctm);
    if (--this->_cRef) {
	return this->_cRef;
...
STDMETHODIMP_(ULONG) DropExt_SXI_Release(LPSHELLEXTINIT psxi)
{
    PSAMPLEDROPEXT this = PSXI2PSMX(psxi);
