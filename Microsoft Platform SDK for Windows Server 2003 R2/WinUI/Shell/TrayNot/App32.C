...
void StateChange(HWND hDlg, UINT_PTR uIndex, UINT uSelect)
{
	...
	uTip = (uSelect==uState1) ? g_sDlgItems[uIndex].uTip1 : g_sDlgItems[uIndex].uTip2;

	if (!GetDlgItemText(hDlg, uTip, szTip, ARRAYSIZE(szTip))
		&& !LoadString(g_hinst, uSelect, szTip, ARRAYSIZE(szTip)))
	{
		*szTip = '\0';
	}

	// The ID of window after the radio button is the ID of the icon
...
void NotifyAdd(HWND hDlg, UINT_PTR uIndex)
{
...
void NotifyChange(HWND hDlg, UINT uIndex)
{
	...
	{
		uDelay = GetDlgItemInt(hDlg, g_sDlgItems[uIndex].uDelayID, &bTranslated, FALSE);
	...
	for (uIndex=0; uIndex<ARRAYSIZE(g_sDlgItems); ++uIndex)
	{
		if (IsDlgButtonChecked(hDlg, g_sDlgItems[uIndex].uNotify))
		{
			bEnable = TRUE;
...
LRESULT CALLBACK TestAppDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
	case WM_INITDIALOG:
